...
from app.extensions import jwt, jwt_required, get_jwt_identity
...
class PlaceResource(Resource):
    ...
    @api.response(404, 'Place not found')
    @api.response(400, 'Invalid input data')
    @api.response(403, 'Only owner can modify the place')
    @jwt_required()
    def put(self,user_id, place_id):
        """checks if requestor and owner are the same account"""
        current_user = get_jwt_identity()
        requestor_id = current_user["id"]
        place = facade.get_place(place_id)
        owner_id = place["owner_id"]
        if requestor_id != owner_id:
            return {"message" : "Only the owner can modify the place"}, 403

        """extract payload then create place instance"""
        place_data = api.payload
        try:
